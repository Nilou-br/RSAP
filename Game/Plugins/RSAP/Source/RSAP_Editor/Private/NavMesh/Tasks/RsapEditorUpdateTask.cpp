...
#include "..\..\..\Public\NavMesh\Tasks\RsapEditorUpdateTask.h"
 ...
 */
layer_idx CalculateOptimalStartingLayer(const FChangedBounds& ChangedBounds)
{
	...
	const int32 MaxSide = ChangedBounds.Current.IsValid()
		? ChangedBounds.Current.GetLengths().GetLargestAxis() : ChangedBounds.Previous.GetLengths().GetLargestAxis();
...
{
#if WITH_EDITOR
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("FRsapEditorUpdateTask ::Run");
	...
	for (const auto& [ActorKey, Pair] : UpdatedActorMap)
	{
		...
		const layer_idx StartingLayerIdx = CalculateOptimalStartingLayer(FChangedBounds(PreviousBoundsList.back(), CurrentBounds));
 ...
 */
bool FRsapEditorUpdateTask::StartReRasterizeNode(const FChunk& Chunk, const node_morton MortonCode, const layer_idx LayerIdx, const rsap_direction RelationsToUpdate)
{
...
void FRsapEditorUpdateTask::RecursiveReRasterizeNode(const UWorld* World, const FChunk& Chunk, FNodePair& NodePair, const layer_idx LayerIdx, const FNodeVector MortonLocation)
{
 ...
 * @return False if the starting Node is occluded. True otherwise.
 */
bool FRsapEditorUpdateTask::StartClearUnoccludedChildrenOfNode(const FChunk& Chunk, const node_morton MortonCode, const layer_idx LayerIdx, const rsap_direction RelationsToUpdate)
{
...
void FRsapEditorUpdateTask::RecursiveClearUnoccludedChildren(const FChunk& Chunk, const FNodePair& NodePair, const layer_idx LayerIdx, const rsap_direction RelationsToUpdate)
{
 ...
 */
void FRsapEditorUpdateTask::StartClearAllChildrenOfNode(const FChunk& Chunk, const node_morton NodeMortonCode, const layer_idx LayerIdx, const rsap_direction RelationsToUpdate)
{
...
void FRsapEditorUpdateTask::RecursiveClearAllChildren(const FChunk& Chunk, const FNodePair& NodePair, const layer_idx LayerIdx)
{
...
void FRsapEditorUpdateTask::InitializeParents(const FChunk& Chunk, const node_morton MortonCode, const layer_idx LayerIdx)
{
 ...
 */
void FRsapEditorUpdateTask::TryUnRasterizeNodes(const FChunk& Chunk, const std::unordered_set<node_morton>& MortonCodes, const layer_idx LayerIdx)
{
...
void FRsapEditorUpdateTask::SetNegativeNeighbourRelations(const FChunk& Chunk)
{
